...
갱신: 2026-09-25. `v0.1.0`과 `v0.1.2`는 철회했고 태그를 재사용하지 않는다. `worktree-cli 0.1.2` crate는 yank됐고 GitHub Release와 tag도 삭제했다. `v0.1.3` tag는 남아 있지만 Linux 빌드에서 Cargo 설치 단계가 건너뛰어져 릴리스 workflow가 실패했다. crate 게시와 GitHub Release 생성 전 실패했으므로 tag는 감사 기록으로 보존하고 재사용하지 않는다. PR #10에서 workflow를 수정했다. `0.1.4`를 대체 버전으로 사용하며 최신 `main`의 nixpkgs pin은 `34ca302`다. `v0.1.1`은 기존 GitHub 릴리스로 남아 있다.

## 배포 경로

- crates.io: `cargo install worktree-cli --version 0.1.4 --locked`
- macOS와 Linux: cargo-dist 셸 설치기 `worktree-cli-installer.sh`
- NixOS와 선언적 설치: Flake 참조 `github:baleen37/worktree/v0.1.4`
...
- macOS ARM64와 Linux x86_64/ARM64에서 태그 고정 설치기로 `wt --help`를 확인한다. Nix는 `nix profile install github:baleen37/worktree/v0.1.4`로 확인한다.
...
`worktree-cli 0.1.2`는 tag의 오래된 nixpkgs pin 때문에 2026-09-25에 yank했고, GitHub Release와 tag를 삭제했다. Yank는 기존 lockfile에서 해당 버전을 사용하는 일을 막지 않으며, 이미 다운로드된 GitHub asset도 회수하지 못한다.

`v0.1.3`은 tag를 보존한다. 2026-09-25 릴리스 workflow가 Linux 컨테이너에서 Cargo를 찾지 못해 crate 게시와 GitHub Release 생성 전에 실패했다. 수정된 workflow가 병합됐으며, `0.1.4`가 최신 nixpkgs lock을 포함하는 대체 버전이다.
