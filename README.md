...
After `worktree-cli` v0.1.5 is published to crates.io:

```sh
cargo install worktree-cli --version 0.1.5 --locked
```

### macOS and Linux

After the `v0.1.5` GitHub Release is published, install that pinned version with the cargo-dist shell installer:

```bash
curl --proto '=https' --tlsv1.2 -LsSf \
  https://github.com/baleen37/worktree/releases/download/v0.1.5/worktree-cli-installer.sh | sh
```
...
```bash
nix profile install github:baleen37/worktree/v0.1.5
```
...
Replace `v0.1.5` with the release tag you want to install. `gh release download` fetches the unified checksum file and all platform archives so `sha256.sum` can verify every archive.
